...
class Board:
    ...
    def get_line_points(self, reading: List[str]) -> List[Tuple[int, int]]:
        ...
        if reading_int[1] == reading_int[3]:
            y_pos = reading_int[1]
...
if __name__ == '__main__':
    not_diagonal_readings = [
        reading for reading in read_input('inputexample') if not is_diagonal(reading)]

    board = Board(not_diagonal_readings)
    board.process_readings(not_diagonal_readings)
