...
class BingoCard:
    ...
    def check_columns(self):
        for column_number in range(self.matrix_dimensions[1]):
            correct_numbers = sum(
                [
                    matrix_cell.has_appeared
                    for matrix_cell in self._get_matrix_column_x(column_number)
                ]
            )
...
if __name__ == '__main__':
    ...
    for drawn_number in drawn_numbers:
        ...
        for bingo_card in bingo_cards:
            if bingo_card not in winner_cards:
    ...
    last_bingo_card_to_win = winner_cards[-1]
    print(f'Last bingo card to win:\n{last_bingo_card_to_win}')
    print(last_winner_number)
    unmarked_numbers_sum = last_bingo_card_to_win.sum_unmarked_numbers()
    final_multiplication = unmarked_numbers_sum*last_winner_number
